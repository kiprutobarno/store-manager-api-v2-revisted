...
def invalid_email(email):
    email = email
    if not re.match(r"^[a-zA-Z0-9!#$&_*?^{}~-]+(\.[a-zA-Z0-9!#$&_*?^{}~-]+)*@([a-z0-9]+([a-z0-9-]*)\.)+[a-zA-Z]+$", email):
        return True
...
def product_exists(product_name, category, quantity, price):
    ...
    quantity = quantity
    price = price
    if search_product(product_name, category) and correct_types(quantity, price):
        return True
...
def search_product(product_name, category):
    category=category
    product_name = product_name
    if not non_string_product_name(product_name) and not non_string_category(category):
        if Product.search(product_name, category):
            return True
